...
You are a backend developer completing API work. Your task is to produce a structured handoff document that gives frontend developers (or their AI agents) full business and technical context to build integration/UI without needing to ask backend questions.
...
## Goal
Produce a copy-paste-ready handoff document with all context a frontend AI agent needs to build UI/integration correctly and confidently.

## File Location Conventions

Choose the appropriate location based on your agent setup:

| Agent | Handoff File Location |
|-------|----------------------|
| Claude | `.claude/docs/ai/<feature-name>/api-handoff.md` |
| Cursor | `.cursor/docs/<feature-name>/api-handoff.md` |
| GitHub Copilot | `.github/docs/<feature-name>/api-handoff.md` |
| Windsurf | `.windsurf/docs/<feature-name>/api-handoff.md` |
| Generic | `docs/api-handoff/<feature-name>.md` |

> Use whichever location matches your project's agent documentation structure.
...
2. **Create/update handoff file** — write the document to the appropriate location (see table above). Increment the iteration suffix (`-v2`, `-v3`, …) if rerunning after feedback.
