...
def create_arrow(
    ...
    # Arrow element (optionally bound to elements)
    arrow = {
    ...
    # Optional label
    if label:
        # Text sizing based on reliable-diagrams.md:
        # width >= maxLineLen * fontSize * 0.7 + padding, height >= fontSize * 1.2 * lines * 1.1
        font_size = 14
        lines = label.split("\n")
        ...
        mid_x = (from_x + to_x) / 2 - width / 2
        mid_y = (from_y + to_y) / 2 - height / 2
...
def add_arrow_to_diagram(
    ...
    if from_id or to_id:
        print(f"  With bindings: from_id={from_id!r}, to_id={to_id!r}")
    arrow_elements = create_arrow(from_x, from_y, to_x, to_y, style, color, label, from_id, to_id)
...
def main():
    ...
    if len(sys.argv) < 6:
        ...
        print("  --from-id ELEMENT_ID             Bind arrow start to elementId")
        print("  --to-id ELEMENT_ID               Bind arrow end to elementId")
    ...
    try:
        ...
        add_arrow_to_diagram(work_path, from_x, from_y, to_x, to_y, style, color, label, from_id, to_id)
